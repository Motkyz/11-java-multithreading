package org.example.multithreading.task02;
...
public class PoolExample {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            // Проверяем, что очередь пула не переполнена
            while (executor.getQueue().remainingCapacity() == 0) {
                Thread.sleep(50); // Ждем, пока освободится место в очереди
            }
        ...
        executor.shutdown();
        executor.awaitTermination(1, TimeUnit.HOURS); // Ждем завершения всех задач
    }
}